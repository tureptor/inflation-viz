 ...
 */
	...
	const root = d3.pack().size([width, height]).padding(0)(
		d3
			.hierarchy(data)
			.sum(value)
	...
	const translateCoordsPrev = (d) =>
		`translate(${(d.x - prevView[0]) * kPrev},${(d.y - prevView[1]) * kPrev})`;
	const translateCoordsNew = (d) =>
		`translate(${(d.x - view[0]) * kNew},${(d.y - view[1]) * kNew})`;
	...
	const labelsG = oldLabelsG.empty()
		? parent.append("g").attr("class", "labels")
		: oldLabelsG;
	...
	const circles = circlesG.selectAll("circle").data(nodes, (d) => d.data.id);
	...
	const parentIsfocusedNode = (d) =>
		d.parent && d.parent.data.name === focusedNode.data.name;
	...
	function handleMouseOver(event, d) {
		...
		// fixes bug where if you don't move cursor, clicking/zooming in keeps old circle tooltip.
	...
	function handleMouseOut(event) {
		d3.select(event.target).attr("stroke-width", 0.1).style("filter", "none");
